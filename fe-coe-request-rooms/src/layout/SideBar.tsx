...
const SideBar = () => {
  ...
  const menuItems = [
    { title: "Dashboard", icon: faTableCells },
    { title: "Image analyse", icon: faImage },
    { title: "Report table", icon: faTableList },
    { title: "Manage room", icon: faDoorOpen },
  ];

  return (
    <div className={`${isOpen ? "w-74" : "w-20"} duration-300 h-screen bg-white shadow-md p-8 flex flex-col`}>
      {/* Header */}
        ...
        {menuItems.map((menu, index) => (
          <li key={index} className="mb-4">
            <a href="#" className={`
              ...
              ${index === 0 ? 'bg-[var(--primary-color)]  text-white' : 'text-gray-700 hover:bg-gray-100'}
              ${!isOpen && 'justify-center'}
            `}>
              <FontAwesomeIcon icon={menu.icon} className={`${!isOpen ? 'text-xl' : ''}`} />
      ...
      <div className="pt-6 border-t border-gray-200">
        <a href="#" className={`
          ...
          ${!isOpen && 'justify-center'}
        `}>
          <FontAwesomeIcon icon={faRightFromBracket} className={`${!isOpen ? 'text-xl' : ''}`} />
          <span className={`${!isOpen ? 'hidden' : 'block'} whitespace-nowrap `}>
            Sign out
          </span>
        </a>
